...
-- ─── GUI state ────────────────────────────────────────────────────────────────
local GUI = { Visible=false, X=120, Y=80, W=295, H=368, Dragging=false, DragOffset=Vector2.new() }
local COL = {
...
local t4BG,t4,t4L = mkRect(11),mkRect(12),mkText(13,12)  -- Closest Bone
local t5BG,t5,t5L = mkRect(11),mkRect(12),mkText(13,12)  -- Curve Aiming
local t6BG,t6,t6L = mkRect(11),mkRect(12),mkText(13,12)  -- Watermark
...
local s3T,s3F,s3L,s3V = mkRect(11),mkRect(12),mkText(13,12),mkText(13,12) -- Curve Strength
...
local ALL = {
    ...
    t1BG,t1,t1L, t2BG,t2,t2L, t3BG,t3,t3L, t4BG,t4,t4L, t5BG,t5,t5L, t6BG,t6,t6L,
...
local function updateGUI()
    ...
    drawToggle(t4BG,t4,t4L, 118, "Closest Bone",      Aim.ClosestBone)
    drawToggle(t5BG,t5,t5L, 142, "Curve Aiming",      Aim.CurveAiming)
    drawToggle(t6BG,t6,t6L, 166, "Watermark (FPS/Ping)", Visuals.Watermark)

    dSep2.From=Vector2.new(x+8,y+189) dSep2.To=Vector2.new(x+w-8,y+189) dSep2.Color=COL.Slider dSep2.Thickness=1 dSep2.Visible=true

    -- Sliders (197, 230, 263)
    local s1pct = (Aim.Smoothness-0.01)/(0.3-0.01)
    drawSlider(s1T,s1F,s1L,s1V, 197, "Legit Smoothness", s1pct, tostring(math.floor(Aim.Smoothness*100)/100))

    local s2pct = (Aim.FOV_Radius-20)/(300-20)
    drawSlider(s2T,s2F,s2L,s2V, 230, "FOV Radius", s2pct, tostring(math.floor(Aim.FOV_Radius)))

    local s3pct = (Aim.CurveStrength-0.1)/(3.0-0.1)
    drawSlider(s3T,s3F,s3L,s3V, 263, "Curve Strength", s3pct, tostring(math.floor(Aim.CurveStrength*100)/100))

    dSep3.From=Vector2.new(x+8,y+297) dSep3.To=Vector2.new(x+w-8,y+297) dSep3.Color=COL.Slider dSep3.Thickness=1 dSep3.Visible=true

    -- Aim Part
    dPartLbl.Position=Vector2.new(x+12,y+305) dPartLbl.Text="Aim Part (if no closest):" dPartLbl.Color=COL.Sub dPartLbl.Visible=true
    dPartBtn.Position=Vector2.new(x+135,y+303) dPartBtn.Size=Vector2.new(110,18) dPartBtn.Color=COL.Panel dPartBtn.Visible=true
    dPartBtnL.Position=Vector2.new(x+140,y+305) dPartBtnL.Text=Aim.AimPart dPartBtnL.Color=COL.Accent dPartBtnL.Visible=true

    -- Unload button
    dUnloadBtn.Position=Vector2.new(x+12,y+333) dUnloadBtn.Size=Vector2.new(w-24,22)
    dUnloadBtn.Color=COL.Danger dUnloadBtn.Transparency=0.35 dUnloadBtn.Visible=true
    dUnloadLbl.Position=Vector2.new(x+w/2-50,y+337) dUnloadLbl.Text="⏏  UNLOAD  (lub DELETE)" dUnloadLbl.Color=COL.Text dUnloadLbl.Visible=true
end
...
reg(UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        ...
        if inBox(mx,my, x,y+118, w,20) then Aim.ClosestBone      = not Aim.ClosestBone      updateGUI() end
        if inBox(mx,my, x,y+142, w,20) then Aim.CurveAiming      = not Aim.CurveAiming      updateGUI() end
        if inBox(mx,my, x,y+166, w,20) then Visuals.Watermark    = not Visuals.Watermark    updateGUI() end

        if inBox(mx,my, x+12,y+197, SLIDER_W,30) then sliderActive=1 end
        if inBox(mx,my, x+12,y+230, SLIDER_W,30) then sliderActive=2 end
        if inBox(mx,my, x+12,y+263, SLIDER_W,30) then sliderActive=3 end

        if inBox(mx,my, x+135,y+303, 110,18) then
            local p={"Head","HumanoidRootPart","UpperTorso"}
            Aim.AimPart=p[(table.find(p,Aim.AimPart) or 1)%3+1] updateGUI()
        end

        if inBox(mx,my, x+12,y+333, w-24,22) then
            if getgenv().ScoutUnload then getgenv().ScoutUnload() end
